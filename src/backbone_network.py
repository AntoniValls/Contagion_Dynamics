...
def compute_disparity_filter_directed(G, weight='weight'):
    ...
    N = nx.DiGraph()
    N.add_nodes_from(G.nodes(data=True), desc="Computing DF")
    for u in tqdm(G):
...
def compute_disparity_filter_undirected(G, weight='weight'):
    '''See docstring for `compute_disparity_filter`.'''
    B = nx.Graph()
    B.add_nodes_from(G.nodes(data=True), desc="Computing DF")
    for u in tqdm(G):
        k = len(G[u])
        if k > 1:
...
def apply_disparity_filter_directed(G, alpha_t=0.8, cut_mode='or'):
    ...
    B = nx.DiGraph()
    B.add_nodes_from(G.nodes(data=True), desc="Applying DF")
    for u, v, w in tqdm(G.edges(data=True)):
        try:
            alpha_in = w['alpha_in']
...
def apply_disparity_filter_undirected(G, alpha_t=0.8):
    ...
    for u, v, w in tqdm(G.edges(data=True), desc="Applying DF"):
